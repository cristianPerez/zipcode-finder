...
const findAndReturnSearch = async (zipcode: string, country: string) => {
  const searchFiltered = await Search.findOne(search => search.zipcode === zipcode && search.countryAbb === country)
...
export const resolvers = {
  Query: {
    search: (_root, { zipcode, country }) => findAndReturnSearch(zipcode, country),
    searches: () => Search.findAll(),
  },
  Mutation: {
    createSearch: (_root, { input: { zipcode, country } }) => findAndReturnSearch(zipcode, country),
  },
}
